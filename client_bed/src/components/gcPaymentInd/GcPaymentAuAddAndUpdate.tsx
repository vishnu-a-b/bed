...
const PaymentForm = ({ paymentId }: { paymentId?: string }) => {
  ...
  useEffect(() => {
    if (paymentId) {
      const fetchDetails = async () => {
        try {
          const items: any = await fetchSingleData(paymentId, "generous-payments-ind");
  ...
  const onSubmit = async (data: PaymentFormData) => {
    ...
    try {
      ...
      if (paymentId) {
        const response = await update(paymentData, "generous-payments-ind", paymentId);
        if (response._id) {
        ...
        const response = await create("generous-payments-ind/manual", paymentData);
        if (response._id) {
