...
export default function ViewPayments() {
  ...
  async function getData() {
    try {
      if (paymentId) {
        const response = await Axios.get(`/generous-payments-ind/${paymentId}&sort=-paymentDate`);
        let items = [];
      ...
      const response = await Axios.post(
        `/generous-payments-ind?${limit}${skip}${statusParam}${paymentModeParam}${searchParam}sort=-paymentDate`,
        requestBody,
        {
          headers: {
