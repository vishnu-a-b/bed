...
export default function ViewPayment() {
  ...
  const [paymentHeadData, setPaymentHeadData] =
    useState<PaymentHeadData | null>(null);

  const refresh: boolean = useSelector(
    (state: RootState) => state.update.refresh
  );
  const paymentId: any = useSelector(
    (state: RootState) => state.payment.id
  );
    ...
    <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 text-sm">
      <div className="flex justify-between items-start mb-2">
        ...
        <span className="bg-gray-200 text-gray-700 text-xs px-2 py-1 rounded">
          {country.totalPayments} total
        </span>
      </div>

      <div className="grid grid-cols-2 gap-1 text-xs">
        <StatItem label="Active" value={country.activePayments} />
        <StatItem label="Month" value={country.thisMonthPayments} />
        <StatItem label="Week" value={country.thisWeekPayments} />
        <StatItem label="Today" value={country.thisDayPayments} />
      </div>
  ...
  // Fetch payment head data
  const fetchPaymentHeadData = useCallback(async () => {
    try {
      const response = await Axios.get("payment/payment-head");
      console.log("Payment Head Data:", response);
      const headData = response.data as PaymentHeadData;

      // Calculate totals across all organizations and countries
      let totalPayments = 0;
      let activePayments = 0;
      let thisMonthPayments = 0;
      let thisWeekPayments = 0;
      let totalAmount = 0;
      let thisMonthAmount = 0;
      let thisWeekAmount = 0;

      headData.data.forEach((org) => {
        totalPayments += org.totalpayments;
        activePayments += org.activepayments;
        thisMonthPayments += org.thisMonthpayments;
        thisWeekPayments += org.thisWeekpayments;
      ...
      console.error("Error fetching payment head data:", error);
    }
  }, []);
...
async function getData() {
  try {
    ...
    const response = await Axios.post(
      `/payment/get?limit=Infinity&${skip}${bedFilter}${isActiveFilter}${searchParam}sortBy=paymentDate`,
      requestBody,
      {
        headers: {
  ...
  const totalContribution = data.reduce((sum, payment) => {
  ...
  return (
    <div className="flex flex-col items-center p-4">
      <div className="container mx-auto py-10">
        {/* payment Head Stats - Only shown when no bed is selected */}
        {!bed && paymentHeadData && (
