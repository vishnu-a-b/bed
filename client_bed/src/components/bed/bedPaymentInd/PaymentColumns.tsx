...
const approvePayment = async (paymentId: string, approved: boolean, remarks?: string) => {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/bed-payments-ind/${paymentId}/approve`, {
    method: 'PATCH',
    headers: {
...
export const paymentColumns: ColumnDef<Payment>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const amount = Number(row.getValue("amount")); // Already in rupees
  ...
  return (
    <div className="flex flex-col space-y-6">
      <div className="space-y-4">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="font-semibold text-lg">₹{paymentData.amount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            </div>
            <div>
        ...
        {/* Razorpay Information (for online payments) */}
        {paymentData.paymentMode === "online" && (
          <div className="bg-blue-50 p-4 rounded-lg">
            <h4 className="font-medium mb-3">Razorpay Information</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(paymentData as any).razorpay_order_id && (
                <div>
                  <p className="text-sm text-gray-500">Razorpay Order ID</p>
                  <p className="text-xs font-mono break-all">{(paymentData as any).razorpay_order_id}</p>
                </div>
              )}
              {(paymentData as any).razorpay_payment_id && (
                <div>
                  <p className="text-sm text-gray-500">Razorpay Payment ID</p>
                  <p className="text-xs font-mono break-all">{(paymentData as any).razorpay_payment_id}</p>
                </div>
              )}
              {(paymentData as any).razorpay_signature && (
                <div>
                  <p className="text-sm text-gray-500">Razorpay Signature</p>
                  <p className="text-xs font-mono break-all">{(paymentData as any).razorpay_signature}</p>
                </div>
              )}
              {(paymentData as any).razorpay_status && (
                <div>
                  <p className="text-sm text-gray-500">Razorpay Status</p>
                  <p className="uppercase">{(paymentData as any).razorpay_status}</p>
                </div>
              )}
              {(paymentData as any).razorpay_fee && (
                <div>
                  <p className="text-sm text-gray-500">Razorpay Fee</p>
                  <p>₹{((paymentData as any).razorpay_fee.amount).toFixed(2)}</p>
                </div>
              )}
              {paymentData.net_amount && (
                <div>
                  <p className="text-sm text-gray-500">Net Amount</p>
                  <p className="font-medium">₹{paymentData.net_amount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="flex gap-2 pt-4 border-t">
        <Button
          className="text-sm"
          onClick={() => {
            dispatch(setUpdateUrl("bed-payments-ind"));
